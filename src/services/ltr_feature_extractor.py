...
class LTRFeatureExtractor:
    ...
    def _extract_metadata_features(
        ...
        """Extract tool metadata features."""
        features = {}
        
        # Tool category (if available)
        category = tool.get('category', context.get('category', 'unknown'))
        # We'll use a simple hash to convert category to numeric
        features['tool_category_hash'] = abs(hash(category)) % 100 / 100.0
        
        # Historical usage (from context if available)
        features['tool_popularity'] = context.get('usage_count', 0) / 100.0
        features['tool_recency'] = context.get('days_since_use', 365) / 365.0
